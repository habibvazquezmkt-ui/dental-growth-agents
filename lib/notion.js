...
// Page IDs fijos de agentes (evita búsqueda paginada)
const AGENTE_IDS = {
  Cobros: '324143ab-281f-81ab-9bbe-d23bf5fe6528'
};
...
async function getCobrosActivos() {
  return queryDatabase(DB.COBRANZA, {
    or: [
      { property: 'Status Pago', select: { equals: 'Pendiente' } },
      { property: 'Status Pago', select: { equals: 'Vencido' } }
    ]
  });
}
...
async function actualizarStatusPago(pageId, status, fechaPagoReal = null) {
  const props = { 'Status Pago': { select: { name: status } } };
...
async function isAgenteActivo(nombre) {
  const pageId = AGENTE_IDS[nombre];
  if (!pageId) return false;
  const res = await fetch(`${NOTION_API}/pages/${pageId}`, { headers: getHeaders() });
  const page = await res.json();
  return page?.properties?.['Estado Operativo']?.select?.name === 'Activo';
}
