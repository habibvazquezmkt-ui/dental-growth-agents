...
const getH = () => ({ 'Authorization': `Bearer ${process.env.NOTION_API_KEY || process.env.NOTION_TOKEN}`, 'Notion-Version': '2022-06-28', 'Content-Type': 'application/json' });
const DB = { CLINICAS: 'bfbaf930-3746-42e7-b52c-6a819cd9264f', COBRANZA: '76bcf3a2-71c8-456a-bcd8-a9de22d30447', CANALES: '74578322-6cb5-463d-b0fb-3084c43b2e6c' };
...
module.exports = async function handler(req, res) {
  ...
  const nombreCanal = `${prod ? 'soporte' : 'test'}_${clinica.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '').substring(0, 60)}`;
  const log = [];
  ...
  try {
    ...
    const slackError = slackRes?.error || null;
    log.push({ paso: 'Slack', ok: !!slackId, id: slackId, error: slackError });

    // 2. Alta en Clínicas — nombres exactos del schema
    const cp = await fetch(`${NOTION_API}/pages`, {
      method: 'POST', headers: getH(),
      body: JSON.stringify({ parent: { database_id: DB.CLINICAS }, properties: {
    ...
    // 4. Registrar en Canales de Comunicación
    const canp = await fetch(`${NOTION_API}/pages`, {
      method: 'POST', headers: getH(),
      body: JSON.stringify({ parent: { database_id: DB.CANALES }, properties: {
        'Clínica': { title: [{ text: { content: clinica } }] },
    ...
    // 7. Bienvenida en canal cliente (solo producción y si el canal se creó)
    if (prod && slackId) {
